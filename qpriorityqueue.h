...
template <class T, typename LessThan = qLess < T > >
QDebug operator<< (QDebug d, const QPriorityQueue<T, LessThan> &q) {
    QPriorityQueue<T, LessThan> t = q;
    d.noquote().nospace() << "QPriorityQueue(";
    while(!t.isEmpty()) {
        d.noquote().nospace() << t.takeFirst() << ((t.size()<2)?"":", ");
    }
    d.noquote().nospace() << ")";
    return d.maybeSpace();
}
